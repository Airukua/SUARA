...
import torch
from model.inference import generate_sample
from model.optim import Muon
...
def _build_optimizer(model, train_config):
    optimizer_name = getattr(train_config, "optimizer", "muon").lower()
    if optimizer_name == "muon":
        return Muon(
            model.parameters(),
            lr=train_config.learning_rate,
            weight_decay=train_config.weight_decay,
            momentum=getattr(train_config, "muon_momentum", 0.95),
            nesterov=getattr(train_config, "muon_nesterov", True),
            adam_betas=(train_config.adam_beta1, train_config.adam_beta2),
        )
    ...
    raise ValueError(f"optimizer tidak didukung: {optimizer_name}")
